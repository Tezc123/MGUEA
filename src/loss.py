...
class icl_loss(nn.Module):
    ...
    def custom_logsoftmax(self, target, logits):
        max_values, _ = torch.max(logits, dim=1, keepdim=True)
        logsoftmax_variant = logits - max_values - torch.log(torch.sum(torch.exp(logits - max_values), dim=1, keepdim=True))
        return -(target * logsoftmax_variant).sum() / logits.shape[0]
