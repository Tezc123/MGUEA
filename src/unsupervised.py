...
class MCLEA:
    ...
    def parse_options(parser):
        parser.add_argument("--file_dir", type=str, default="data/DBP15K/zh_en", required=False,
                            help="input dataset file directory, ('data/DBP15K/zh_en', 'data/DBP15K/ja_en')")
        ...
        parser.add_argument("--check_point", type=int, default=50, help="check point")
        ...
        parser.add_argument("--il_start", type=int, default=300, help="If Il, when to start?")
        parser.add_argument("--bsize", type=int, default=512, help="batch size")
        parser.add_argument("--unsup", action="store_true", default=True)
        parser.add_argument("--topk", type=int, default=7000, help="initial alignment seeds")
        parser.add_argument("--topl", type=int, default=5, help="structure-enhanced iterative strategy")
        # parser.add_argument("--long_tail_analysis", action="store_true", default=False)
        parser.add_argument("--lta_split", type=int, default=300, help="split in {0,1,2,3,|splits|-1}")
    ...
    def visual_pivot_induction(self, mode="img"):
        feat = torch.cat((self.img_features, self.name_features, self.char_features), dim=1)
        l_f = F.normalize(feat[self.left_ents])
        r_f = F.normalize(feat[self.right_ents])
        sim = l_f.mm(r_f.t())  # t : transpose
        topk = self.args.topk
        two_d_indices = get_topk_indices(sim, topk * 100)
        del l_f, r_f, sim, feat
        ...
        print("train links length: %d" % (len(visual_links)))
    ...
    def init_data(self):
        ...
        self.ills_ = np.array(self.ills, dtype=np.int32)
        self.neighbor_dict = get_neighbor_dict(self.triples)
        ...
        if "DBP15K" in file_dir:
            ...
            self.name_features = F.normalize(torch.Tensor(self.ent_vec)).to(device)
        ...
        self.rel_features = F.normalize(torch.Tensor(self.rel_features)).to(device)
        ...
        self.att_features = F.normalize(torch.Tensor(self.att_features)).to(device)
    ...
    def semi_supervised_learning(self):
        ...
        topk_indices_l = np.argsort(distance.cpu().numpy(), axis=1)[:, :self.args.topl]
        topk_indices_r = np.argsort(distance.t().cpu().numpy(), axis=1)[:, :self.args.topl]
        preds_l = torch.argmin(distance, dim=1).cpu().numpy().tolist()
        preds_r = torch.argmin(distance.t(), dim=1).cpu().numpy().tolist()
        del distance_list, distance, img_emb, rel_emb, att_emb, name_emb, char_emb, joint_emb, final_emb, gph_emb
        return preds_l, preds_r, topk_indices_l, topk_indices_r

    def un_supervised_learning(self):
        with torch.no_grad():
        ...
        distance_list = []
        for i in np.arange(0, len(self.left_ents), 1000):
            d = pairwise_distances(final_emb[self.left_ents[i:i + 1000]], final_emb[self.right_ents])
            distance_list.append(d)
        distance = torch.cat(distance_list, dim=0)
        preds_l = torch.argmin(distance, dim=1).cpu().numpy().tolist()
        preds_r = torch.argmin(distance.t(), dim=1).cpu().numpy().tolist()
        del distance_list, distance, gph_emb, img_emb, rel_emb, att_emb, name_emb, char_emb, joint_emb, final_emb
    ...
    def kl_alignment_loss(self, joint_emb, gph_emb, rel_emb, att_emb, img_emb, name_emb, char_emb, train_ill):
        ...
        total_loss = self.align_multi_loss_layer([loss_GCN, loss_rel, loss_att, loss_img, loss_name, loss_char]) * zoom
        return total_loss
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            if self.args.unsup and (epoch + 1) % self.args.semi_learn_step == 0 and 0 <= epoch <= 49:
                preds_l, preds_r = self.un_supervised_learning()
                links = [(self.left_ents[i], self.right_ents[p]) for i, p in enumerate(preds_l) if
                         (preds_r[p] == i) and ((self.left_ents[i], self.right_ents[p]) in links)]
                print("[epoch %d] #links in candidate set: %d" % (epoch, len(links)))

            if self.args.unsup and epoch == 49:
                self.train_ill = np.array(links, dtype=np.int32)
                self.left_non_train = list(set(self.left_ents) - set(self.train_ill[:, 0].tolist()))
                self.right_non_train = list(set(self.right_ents) - set(self.train_ill[:, 1].tolist()))
                
            if epoch >= self.args.il_start and (epoch + 1) % self.args.semi_learn_step == 0 and self.args.il:
                preds_l, preds_r, topk_indices_l, topk_indices_r = self.semi_supervised_learning()
                ...
                if (epoch + 1) % (self.args.semi_learn_step * 10) == self.args.semi_learn_step:
                    ...
                    for i, p in enumerate(preds_l):
                        if preds_r[p] == i and (self.left_non_train[i], self.right_non_train[p]) in new_links:
                            resu = getSimResult(self.left_non_train, self.right_non_train, i, p, topk_indices_l, topk_indices_r, self.train_ill, self.neighbor_dict)
                            if resu:
                                n.append((self.left_non_train[i], self.right_non_train[p]))
                    new_links = n
            ...
            if epoch >= self.args.il_start and (epoch + 1) % (self.args.semi_learn_step * 10) == 0 and len(
                ...
                self.logger.info("new_links_elect: {:d}".format(len(new_links_elect)))

                self.train_ill = np.vstack((self.train_ill, np.array(new_links_elect)))
                self.logger.info("train_ill length: {:d}".format(self.train_ill.shape[0]))
                
                num_true = 0
                for i, nl in enumerate(new_links_elect):
                    if nl in self.test_ill_:
                        num_true += 1
                self.logger.info("true_links: {:d}".format(num_true))
                self.logger.info("true link ratio: {:.2f}%".format(100 * num_true / len(new_links_elect)))
            ...
            if (epoch + 1) % self.args.check_point == 0:
                self.logger.info("epoch {:d} checkpoint!".format(epoch))
                self.test(epoch)
    ...
    def test(self, epoch):
        with torch.no_grad():
            ...
            self.logger.info(
                "l2r: acc of top {} = {}, mr = {:.3f}, mrr = {:.3f}, time = {:.4f} s ".format(top_k, acc_l2r, mean_l2r,
                                                                                              mrr_l2r,
                                                                                              time.time() - t_test))
            self.logger.info(
                "r2l: acc of top {} = {}, mr = {:.3f}, mrr = {:.3f}, time = {:.4f} s ".format(top_k, acc_r2l, mean_r2l,
                                                                                              mrr_r2l,
                                                                                              time.time() - t_test))
