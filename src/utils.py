...
def get_neighbor_dict(triples):
    neighbor_dict = {}
    for tri in triples:
        if tri[0] not in neighbor_dict:
            neighbor_dict[tri[0]] = []
        if tri[2] not in neighbor_dict[tri[0]]:
            neighbor_dict[tri[0]].append(tri[2])

        if tri[2] not in neighbor_dict:
            neighbor_dict[tri[2]] = []
        if tri[0] not in neighbor_dict[tri[2]]:
            neighbor_dict[tri[2]].append(tri[0])
    return neighbor_dict

def getSimResult(left_non_train, right_non_train, i, p, topk_indices_l, topk_indices_r, train_ill, neighbor_dict):
    leftId = left_non_train[i]
    rightId = right_non_train[p]

    list_l = [right_non_train[item] for item in topk_indices_l[i]]
    list_r = [left_non_train[item] for item in topk_indices_r[p]]
    assert rightId == list_l[0]
    assert leftId == list_r[0]

    sim_1 = []
    for l1 in list_l:
        sim_1.append(getSim(leftId, l1, train_ill, neighbor_dict))
    sim_2 = []
    for l2 in list_r:
        sim_2.append(getSim(rightId, l2, train_ill, neighbor_dict))

    if sim_1.index(max(sim_1)) == 0 and sim_2.index(max(sim_2)) == 0:
        return True
    else:
        return False


def getSim(Id, itemId, train_ill, neighbor_dict):
    left = train_ill[:, 0].tolist()
    left_dict_list = neighbor_dict[Id]
    right_dict_list = neighbor_dict[itemId]
    count = 0
    for i in left_dict_list:
        if i in left:
            index = left.index(i)
            i_align = train_ill[index][1]
            if i_align in right_dict_list:
                count += 1
    sim = count
    # Jaccard相似性
    # sim = count / (len(left_dict_list) + len(right_dict_list) - count)
    return sim
